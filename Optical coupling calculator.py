...
def calculate_results():
    try:
        ...
        eta_total_value = round(eta_dx_value * eta_dy_value * eta_zm_value, 4)
...
ttk.Label(root, text="delta_z (μm):").grid(row=6, column=0)
entry_delta_z_var = tk.StringVar(value="0")
entry_delta_z = ttk.Entry(root, textvariable=entry_delta_z_var)
entry_delta_z.grid(row=6, column=1)


ttk.Label(root, text="Wavelength (μm):").grid(row=7, column=0)
entry_wavelength_var = tk.StringVar(value="1.55")
entry_wavelength = ttk.Entry(root, textvariable=entry_wavelength_var)
entry_wavelength.grid(row=7, column=1)

# Create and place the result labels
ttk.Label(root, text="η_M:").grid(row=8, column=0)
result_eta_M = tk.StringVar()
ttk.Label(root, textvariable=result_eta_M).grid(row=8, column=1)

ttk.Label(root, text="η_δx:").grid(row=9, column=0)
result_eta_delta_x = tk.StringVar()
ttk.Label(root, textvariable=result_eta_delta_x).grid(row=9, column=1)

ttk.Label(root, text="η_δy:").grid(row=10, column=0)
result_eta_delta_y = tk.StringVar()
ttk.Label(root, textvariable=result_eta_delta_y).grid(row=10, column=1)

ttk.Label(root, text="η_ZM:").grid(row=11, column=0)
result_eta_ZM = tk.StringVar()
ttk.Label(root, textvariable=result_eta_ZM).grid(row=11, column=1)

ttk.Label(root, text="η_total:").grid(row=12, column=0)
result_eta_total = tk.StringVar()
ttk.Label(root, textvariable=result_eta_total).grid(row=12, column=1)
...
check_dB.grid(row=14, columnspan=2)

# Create and place the heatmap button
ttk.Button(root, text="Generate IL map (2D)", command=generate_heatmap).grid(row=15, column=0)
ttk.Button(root, text="Generate IL map (1D)", command=generate_1D_IL_map).grid(row=15, column=1)

#label button
ttk.Button(root, text="Labels", command=open_label_settings).grid(row=15, column=2)

# IL vs Δz
ttk.Button(root, text="Generate IL vs Δz", command=generate_IL_vs_dz).grid(row=16, column=0, columnspan=3)
